...
function DashboardContent() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Kreiranje radnje nije uspjelo.");
    } finally {
      setCreating(false);
    }
  }
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.25em] text-stub">Pregled</p>
            <h1 className="mt-1 font-display text-3xl italic text-ink">Moji saloni</h1>
          </div>
          ...
          <Card className="mt-6 p-6">
            <form onSubmit={handleCreate} className="flex flex-col gap-4">
              ...
              <div className="relative">
                ...
                {latitude && longitude && (
                  <p className="mt-1 text-xs text-green-600">✓ Lokacija uspješno prepoznata na mapi</p>
                )}
              </div>
              ...
              <div className="border-t border-line pt-4 mt-2">
                <h3 className="text-sm font-semibold text-ink mb-3">Radno vrijeme</h3>
                <div className="grid gap-3">
                  {Object.entries(workingHours).map(([day, hours]: [string, any]) => (
                    <div key={day} className="flex items-center justify-between gap-2 text-sm bg-gray-50 p-2.5 rounded-sm border border-line">
                      <div className="flex items-center gap-2 w-32">
                        <input
                          type="checkbox"
                          checked={hours.is_working}
                          onChange={(e) => handleWorkingHourChange(day, "is_working", e.target.checked)}
              ...
              <Button type="submit" loading={creating} className="self-start mt-2">
                Sačuvaj radnju
              </Button>
            </form>
          </Card>
          ...
          {loading ? (
            ...
            <Card className="p-10 text-center">
              <p className="text-sm text-muted">
                Još uvijek nemate nijednu radnju. Kreirajte prvu da počnete primati rezervacije.
              </p>
            </Card>
          ) : (
              ...
              {shops.map((shop) => (
                ...
                >
                  <div>
                    ...
                    <h2 className="mt-1 font-display text-xl text-ink">{shop.name}</h2>
                    {shop.address && (
                    ...
                    {shop.description && (
                      <p className="mt-2 line-clamp-2 text-sm text-muted">{shop.description}</p>
                    )}
                  </div>
